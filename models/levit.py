...
class LeViT_UNet_128s(torch.nn.Module):
    ...
    def __init__(self, img_size=224, ## aping: should change
        ...
        self.cnn_b1 = torch.nn.Sequential(
            Conv2d_BN(5, n // 8, 3, 2, 1, resolution=img_size), activation())
    ...
    def forward(self, x):
        x = x.resize_(x.shape[0], x.shape[1], 224,224)
        ...
        x = F.upsample(logits, size=(256, 256), mode="bilinear")
...
if __name__ == '__main__':
    for name in specification:
        ...
        #net(torch.randn(8, 1, 224, 224)) #NCHW
        # print(net)

        summary(net)
        
        print(net(torch.randn(8, 5, 256, 256)).shape)
